...
namespace MedSCAN.Boundary
{
   ...
   *///---------------------------------------------------------------------------
    public partial class Multi_Dose : Form
    {
        ...
        public Multi_Dose(Entity.Medications med, Entity.PatientMeds patientMed)
        {
            InitializeComponent();
            btnOK.Enabled = false;
        ...
        {
            //amount of medicine that needs to be added
            float addAmount = (dose - str);
            string instructions = "The dose required for this patient is " + dose + " " + uom.ToUpper() +
                                  " you must ADD " + addAmount.ToString("#.###") + uom.ToUpper() +
                                  " to administer the correct dosage.";

            return instructions;
        }
        //indicates how the medication needs to be combined before administering to the patient
        private string Description(float str, float dose, float units)
        {
            // ie 1 unit = 500mg (str) && req dose = 800mg
            float requiredAmount; // dont wanna divide by zero >.<
            if (str == 0)
                requiredAmount = dose;
            else
                requiredAmount = (dose / str);   // ie 800mg / 500mg = 1.6mg
            int wholeUnits = (int)requiredAmount; // (int)1.6mg = 1 => 500 mg (whole tab)
            float partialUnits = requiredAmount - wholeUnits;   //1.6 - 1 = .6 = 500 mg (partial tab) .6 * 500mg = 300mg needed

            float partialAmount = partialUnits * ((int)str);
            string description = "Please administer: " + (int)(wholeUnits) + " unit(s) " +
                                 "and " + partialAmount.ToString("#.###") + " (Partial) units.";
        ...
        private void txtBoxPatientID_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                // ID is Valid
                errorProviderMD.SetError(txtBoxMedIDBC, "");
                btnGive.Enabled = true;
            }
            else
            {
                errorProviderMD.SetError(txtBoxMedIDBC, "Please enter valid Medication ID. (8-15) digits");
                btnGive.Enabled = false;
            }
        }
